...
with col2:
    ...
    if response_amt is None:
        st.metric(label=curr_codes_data[to_currency]['name'], value= response_amt, delta='{:.2f}'.format(delta), delta_color='inverse', help='arrows indicate change from yesterday\'s price')
    else:
        st.metric(label=curr_codes_data[to_currency]['name'], value= response_amt, delta='{:.2f}'.format(delta), delta_color='inverse', help='arrows indicate change from yesterday\'s price' )
